...
class Withdraw(db.Model):
    ...
    usd_value = db.Column(db.Float) # تمت إضافة قيمة الدولار
...
def login():
    ...
    if not user:
        ...
        user = User(id=uid, name=data.get('name'), username=data.get('username'), referrer_id=final_ref)
        db.session.add(user)
        if final_ref:
        ...
        db.session.commit()
    return jsonify({
        "balance": user.balance, "ads_watched": user.ads_watched,
        "referral_count": user.referral_count, "total_withdrawn": user.total_withdrawn,
        "wallet": user.wallet, "language": user.language
    })
...
def watch():
    ...
    if user:
        ...
        if user.last_ad_watched:
            if (now - user.last_ad_watched).total_seconds() < 10: 
        ...
        if user.referrer_id and not user.referral_bonus_paid and user.ads_watched >= ADS_REQUIRED_FOR_REF:
            ...
            if referrer:
                ...
                send_telegram_msg(user.referrer_id, f"💰 مبروك! مكافأة إحالة {REFERRAL_BONUS} نقطة.")
        db.session.commit()
...
def withdraw():
    ...
    amount = int(data.get('amount', 0)) # المبلغ المطلوب سحبه
    ...
    if user and amount >= MIN_WITHDRAW and user.balance >= amount:
        ...
        user.wallet = wallet
        user.balance -= amount # خصم المبلغ فقط
